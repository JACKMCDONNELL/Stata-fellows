...
log using "/Users/johnmcdonnell/Desktop/Stata fellow training/graphics.log", replace
...
* for some graph types, Stata allows you to leave out the "graph" command. In a scatter plot, you can actually leave out the "twoway" command, too
...
* overall look of Stata graph is specified by graph sceme, so changing this can change the look of the graph considerably. 
...
* in addition to these basic graphs, there are 50+ statistical graph commands corresponding to regression diagnostics, time-series graphs, graphs for survival analysis

qnorm income /* I've referenced this special graph type before, it helps to assess for the normality of a variable */
...
* if you specify more than one "y" variable, Stata will mark the different ones with various sympbols to make the difference apparent
...
scatter rent_w rent_e size, msymbol(th oh) msize(*.5 2) // the triangle is now half as large as it was before, and the ecircle is 2% of the graph height
...
* alternately, use the graph editor. it is almost always easier to use the graph editor if you want to make these fine adjustments
* unfotunately, you can save the final product of the graph editor, but can't save the exact path. this is a limitation for reproducibility
...
scatter rent size, xlabel(none) ylabel(minmax) // tells Stata to use no labels on X axis, and only min and max on the y
...
scatter rent size, ytick(minmax) xmtick(##10) // put 2 big tick lines on the yaxis at min and max, make the x small tick lines be 10 inbetween successive
...
twoway histogram rent, fraction name(hy, replace) horizontal yscale(alt) ylabel(0(500)2500, grid gmax) fxsize(25)
